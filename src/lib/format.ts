 ...
 * `names` — карта из активного словаря (dict.evrk), чтобы названия ниш
 * переводились вместе с языком кабинета. Без неё — фолбэк «EVRK NN».
 */
export function evrkName(
  code2: string | null | undefined,
  names?: Record<string, string>,
): string {
  if (!code2) return '—';
  return names?.[code2] ?? `EVRK ${code2}`;
}
